...
def simulador():
    ...
    try:    
        ...
        if (len(idEnergyCommunityProcess) != 0 and len(idEnergyCommunity) != 0 and len(simulacion_fcDesde) != 0 and len(simulacion_fcHasta) != 0):
            ...
            #Paso 7.1: Almacenamiento de los datos registrados (almacenamos en archivo).
            logging.info("\n")
            logging.info(tiem.now().__format__('%d/%m/%Y %H:%M:%S')+" -> Paso 7.1: ComunidadesEnergeticasServicio.almacenarDatosCalculadosTxt: Almacenamos los valores obtenidos en archivo")
            logging.info("\n")
            archivoLectura = ComunidadesEnergeticasServicio.almacenarDatosCalculadosTxt(comunidadEnergetica,Anyo)
            ...
            #Paso 7.3: Correccion del archivo txt para su uso en distribuidoras
            logging.info("\n")
            logging.info(tiem.now().__format__('%d/%m/%Y %H:%M:%S')+" -> Paso 7.3: ComunidadesEnergeticasServicio.almacenarDatosCalculadosTxt: Almacenamos los valores obtenidos en archivo")
            logging.info("\n")
            cedsa.principal(archivoLectura,archivoLectura)
        
            #Paso 8: Establecemos en base de datos como finalizado la ejecución en la correspondiente tabla 
            print("Finalizando la ejecucion y actualizando el registro del proceso")
            logging.info("\n")
