...
def simulador(idEnergyCommunity):
    """
    ...
    try:    
        ...
        try:    
            parametrosSimulacion = ComunidadesEnergeticasServicio.obtenerParametrosEjecucionSimulacion2(agenteEjecucionMySql,Anyo,str(idEnergyCommunity))
        except:
        ...
        if (len(idEnergyCommunityProcess) != 0 and len(idEnergyCommunity) != 0 and len(simulacion_fcDesde) != 0 and len(simulacion_fcHasta) != 0):
            ...
            logging.info("\n")
            ComunidadesEnergeticasServicio.almacenarDatosCalculadosTxt(comunidadEnergetica,Anyo)
